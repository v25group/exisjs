...
When testing an API, you want to boot the app, run HTTP requests against it, and then gracefully shut it down (closing database connections so the test runner doesn't hang). `createTestContext` does exactly this for you automatically!

```typescript filename="tests/users.test.ts"
import { describe, it, assert, createTestContext } from 'exisjs/testing'
import app from '../src/http/server' // Import your Exis instance

describe('Users API', () => {
  // Automatically boots the app, runs `onStart`, and gracefully shuts down databases after tests!
  const client = createTestContext(app)

  it('should create a new user', async () => {
    // client acts as a super-fast in-memory HTTP client
    const res = await client.post('/users', {
      body: { name: 'Alice', email: 'alice@example.com' }
    })

    // Native Node.js assertions
...
describe('Billing API', () => {
  ...
  it('should process payment', async () => {
    const res = await client.post('/billing/charge', {
      body: { amount: 500 }
    })

    assert.strictEqual(res.statusCode, 200)
    assert.strictEqual(res.json.transactionId, 'test_123')
  })
})
```
