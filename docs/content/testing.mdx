...
```typescript filename="tests/setup.ts"
import { beforeEach } from 'exisjs/testing'
import { getActiveApp } from 'exisjs/app'

beforeEach(() => {
  const app = getActiveApp()
  if (app) {
    app.container.clearCache() // Purges all instantiated singletons
  }
})
```

### 2. Zero-Config Singletons (Redis, Prisma, etc.)
...
1. Run your test runner in a mode that isolates processes per file.
2. Manually clear database tables or flush Redis in a `beforeEach` block.
