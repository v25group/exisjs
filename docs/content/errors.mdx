...
Instead of writing manual `try/catch` blocks in every route or calling `app.onError` inside `server.ts`, ExisJS natively discovers and auto-mounts `src/http/error.ts` (or `error.js`).

Export an `onError(err, req, res)` function to centrally format, log, and respond to all unhandled exceptions and validation errors:
 ...
 * Auto-mounted at startup to intercept all unhandled exceptions.
 */
...
> [!TIP]
> You can also scaffold this file instantly via the CLI:
> ```bash
> exis generate error
> # or shorthand:
> exis g error
