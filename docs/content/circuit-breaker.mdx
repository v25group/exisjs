...
2. **`OPEN`** (Failing): The failure threshold has been reached. The circuit is tripped, and all requests are instantly rejected with a `CircuitBreakerError` to protect the system.
...
export default controller({
  charge: route.post('/charge', {
    async handle(req, res) {
      try {
        // 2. Wrap your fragile logic inside .fire()
