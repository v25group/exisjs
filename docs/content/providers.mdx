...
ExisJS utilizes an ultra-fast runtime container system. By default, ExisJS does not aggressively use `reflect-metadata` for automatic constructor injection (unlike NestJS) to maintain a zero-overhead footprint. The `inject()` function is the primary and recommended way to resolve dependencies smoothly.
</Info>
