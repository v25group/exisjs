...
ExisJS provides standard ES6 decorators for developers who prefer the Object-Oriented Programming (OOP) paradigm (inspired by NestJS). 

Because ExisJS does not rely on reflection libraries like `reflect-metadata`, these decorators are fast and memory-efficient.
...
ExisJS allows you to configure your root application and directory-scoped gateways using decorators, replacing the need for functional `exis()` and `defineGateway()` wrappers if you prefer OOP.
...
Instead of manually digging through the `req` object, ExisJS allows you to inject request properties directly into your method parameters.
...
- `@Permissions(...permissions: string[])` - Restricts a route/controller to users with specific permissions.
