...
<Info title="Language Support">
  We highly recommend **TypeScript** for maximum type-safety and automatic
  frontend client generation. However, ExisJS perfectly supports vanilla
  JavaScript as well. ExisJS leverages ultra-fast bundlers under the hood,
  meaning you never have to manually run `tsc` to start your server.
</Info>
...
<div className="my-6 max-w-lg">
  <CopyCommand command="npx @exisjs/create@latest my-app" />
</div>
...
```typescript filename="src/http/route.ts (Functional)"
...
```typescript filename="src/http/route.ts (OOP)"
