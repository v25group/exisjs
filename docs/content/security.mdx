...
export default controller({
  // Only users with req.user.role === 'superadmin' can access this
  deleteDatabase: route.delete('/db', {
    middlewares: [requireRole('superadmin')],
    handle() {
      return { success: true }
    }
  }),
  ...
  viewDashboard: route.get('/dashboard', {
    middlewares: [requireRole(['admin', 'superadmin', 'manager'])],
    handle() {
      return { metrics: [] }
    }
  })
...
```typescript filename="src/http/admin/route.ts"
import { Controller, Delete, Get, Use } from 'exisjs/decorators'
import { requireRole } from 'exisjs/auth'
...
export default class AdminController {
  ...
  // Accepts an array of allowed roles
  @Use(requireRole(['admin', 'superadmin', 'manager']))
  @Get('/dashboard')
  async viewDashboard() {
    return { metrics: [] }
  }
