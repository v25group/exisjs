...
Once your user is authenticated and their payload is attached to `req.user`, ExisJS provides native, declarative Role-Based Access Control right inside the route configuration! You don't need any custom middlewares for this.
...
- `dbSanitize({ mongo: true })`: A powerful universal scraper that strips NoSQL operators (`$where`) to provide robust defense-in-depth for MongoDB databases.
