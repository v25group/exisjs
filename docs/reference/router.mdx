...
- `body`: A validation schema for the request body (e.g., using `tex` or Zod).
- `query`: A validation schema for the query string (e.g., `tex.pagination()`).
- `params`: A validation schema for the URL path parameters.
- `upload` (`string | RouteUploadOptions`): Declarative multipart upload configuration (e.g. `{ field: 'photo', maxBytes: 10 * 1024 * 1024, mimeTypes: [...] }`). Automatically mounts `fileUpload` middleware and populates `req.file` / `req.files`.
- `timeout` (`number`): Per-route timeout override in milliseconds (e.g. `120000`). Overrides global timeout for this endpoint.
...
export default controller({
  getUserInfo: route.get('/info/:id', {
    ...
    query: tex.object({ debug: tex.boolean({ optional: true }) }),
    async handle({ body, query, params, headers, req, res, app, state, user, workspace }) {
      ...
      // 4. `resolve` is the request-scoped dependency injection resolver!
      const usersService = resolve(UsersService)

      // 5. `state` is a mutable object shared across middlewares
      const cacheHit = state.cacheHit 

      // 6. `user` and `workspace` are strictly typed from your custom context!
...
export default class UserController {
  ...
  @Post('/', {
    body: tex.object({ email: tex.email(), password: tex.string() })
  })
...
- `@QueryParam('search')` / `@Query('search')`: Injects a specific query string parameter.
...
- `createParamDecorator(factory)`: Factory to construct custom parameter decorators.
...
- `@Use(...items)`: Unified decorator that applies middlewares, Guards (`canActivate`), Interceptors (`intercept`), and Exception Filters (`catch`) to a route or entire controller.
...
- `@SetMetadata(key, value)`: Attaches arbitrary metadata to classes and handler methods (retrieved via `Reflector`).
- `Reflector`: Reads and merges metadata attached with `@SetMetadata`.
...
## 3. Streaming & Backpressure Pipeline

The response object supports universal streaming with backpressure management across all route types:

- `res.sendStream(source)`: Streams a Node.js `stream.Readable`, Web `ReadableStream`, or `AsyncIterable` directly to the client socket. Pauses consumption when socket buffers fill up and terminates source readers upon client disconnection.
- `res.stream(source)`: Alias for `sendStream()`.
- **Automatic Handler Auto-Piping**: Returning a Node.js stream, Web standard `ReadableStream`, or `AsyncIterable` generator directly from `handle()` automatically delegates to `res.sendStream()`.

---

## 4. `AppRouter` Client Manifest Contract

ExisJS generates a compile-time route manifest in `.exis/types.d.ts` and `.exis/types/index.d.ts`:

- `AppRouterPaths`: Flat dictionary mapping URL route patterns (e.g. `'/users/:id'`) to their strongly-typed handler contracts (`(payload?: B, options?: { query?: Q; params?: P }) => Promise<R>`).
- `AppRouterTree`: Hierarchical dot-notation tree mapping nested folder paths (`users: { ':id': ... }`) for `@exisjs/fetch`'s `createClient<AppRouter>()`.
- `AppRouter`: Combined intersection type `AppRouterPaths & AppRouterTree`.
- `RouteDefinition.__type`: Phantom type property on every route definition preserving exact inferred return types, DTO schemas, and Drizzle/Prisma models.
